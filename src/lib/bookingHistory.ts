...
export async function getBookingHistory(
    ...
    const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/v1/bookings?${queryParams.toString()}`,
        {
            method: "GET",
            headers: {
...
export async function getScheduledBookings(page = 1, limit = 10) {
    return getBookingHistory({ status: "SCHEDULED", page, limit });
}
...
export async function getCancelledBookings(page = 1, limit = 10) {
    return getBookingHistory({ status: "CANCELLED", page, limit });
}
...
export async function getDepartedBookings(page = 1, limit = 10) {
    return getBookingHistory({ status: "DEPARTED", page, limit });
}
