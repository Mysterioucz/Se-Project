...
interface BaggageAllowanceProps {
    ...
    arrivalPlace: string;
    hasReturn?: boolean; // ถ้าไม่ใส่ = false
...
function DepartureAdditionalService({ passengers, departurePlace, arrivalPlace, onChange, }: DepartureProps){
    const [selectedBaggage, setSelectedBaggage] = useState<string[]>(
        Array(passengers.length).fill("Not Included")
    );

    const handleBaggageChange = (index: number, value: string) => {
    ...
    return (
        <div className="flex flex-col p-0 m-0">
            ...
            {/* This part must repeat for all the passengers we have*/}
            {passengers.map((name, index) => (
                ...
                <div className="flex w-[9.625rem] flex-col justify-center items-center py-0 px-[0.5rem] gap-[0.5rem] py-[1rem]">
                    ...
                    <SelectComponent
                        ...
                        value={selectedBaggage[index]}
                        placeholder="Add Baggage"
                        maxChildrenHeight="max-h-[16rem]"
                        onChange={(e) => handleBaggageChange(index, e.target.value as string)}
                    >
...
function ReturnAdditionalService({ passengers, departurePlace, arrivalPlace, onChange, }: DepartureProps) {
    const [selectedBaggage, setSelectedBaggage] = useState<string[]>(
        Array(passengers.length).fill("Not Included")
    );
...
export default function BaggageAllowance({ departurePlace = "Bangkok", arrivalPlace = "Chiang Mai", hasReturn = true, onChange, }: { departurePlace?: string; arrivalPlace?: string; hasReturn?: boolean; onChange?: (baggageSelections: { depart: string[]; return?: string[] }) => void; }) {
